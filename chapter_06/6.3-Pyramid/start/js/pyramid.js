const drawPyramid = (data) => {
  ...
  const binsWomen = d3.bin().value((d) => d.salary)(dataWomen);

  const dataMen = data.filter((d) => d.gender === "Male");
  const binsMen = d3.bin().value((d) => d.salary)(dataMen);

  console.log(binsMen);
  console.log(binsWomen);
